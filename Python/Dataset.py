...
class DatasetClass:
    ...
    def train_val_test_split(self,test_size,seed=None):
        ...
        learn_ids = self.train_df.index
        self.train_ids, self.val_ids = train_test_split(learn_ids,test_size=test_size,stratify=self.train_df.strata,random_state=seed)
        # test_ids = self.df.index.difference(learn_ids)
        
        self.X_train = np.array(self.df.loc[self.train_ids,'image'].values.tolist(),dtype='uint8')
        self.Y_train = np.array(self.train_df.loc[self.train_ids,'masks'].values.tolist(),dtype='bool')
        self.X_feat_train = np.array(self.df.loc[self.train_ids,"z"].values.tolist(),dtype='uint8')
        
        self.X_val = np.array(self.df.loc[self.val_ids,'image'].values.tolist(),dtype='uint8')
        self.Y_val = np.array(self.train_df.loc[self.val_ids,'masks'].values.tolist(),dtype='bool')
        self.X_feat_val = np.array(self.df.loc[self.val_ids,'z'].values.tolist(),dtype='uint8')
