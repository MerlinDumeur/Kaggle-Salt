...
#                 line['name'] = name
                ...
                if 'arch' in keys:
                    ...
                    line = cv(dataset, line, n_splits, test_size, parameters, param, pdf, j)

                else:

                    manager = multiprocessing.Manager()
                    d = manager.dict(line.to_dict())
                    p = Process(target=cv,args=(dataset, line, n_splits, test_size, parameters, param, pdf, j,d))
                    p.start()
                    p.join()
                    line = pd.Series(d)
...
def cv(dataset,line,n_splits,test_size,parameters,param,pdf,j,d_return=None):
