...
#                 line['name'] = name
                ...
                if update_best_para:
                    ...
                    last_loss = np.mean(line['min_val_loss'])
...
def cv(dataset,line,n_splits,parameters,pdf,j,d_return=None):
    ...
    if d_return is not None:
        ...
        param = pdf.loc[j,'arch']

        model = param(**pdf.loc[j,pdf.columns.difference(['arch' + '_name','arch'])].dropna().to_dict())
